...
function resolveSummaryModel(config) {
    const explicit = (0, model_1.normalizeModelName)(config.models.summaryModel);
    if (explicit)
        return explicit;
    const gemini = (0, model_1.normalizeModelName)(config.models.geminiModel);
    if (gemini)
        return gemini;
    const controller = (0, model_1.normalizeModelName)(config.models.controllerModel);
    if (controller)
        return controller;
    return '';
}
async function maybeSummarize(ctx, config, text, label) {
    ...
    try {
        ...
        }), (config.search.summaryTimeout || 15) * 1000, `Summary(${label})`);
