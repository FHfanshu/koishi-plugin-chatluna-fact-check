...
class FactCheckTool extends tools_1.StructuredTool {
    ...
    getConfiguredSources() {
        return (this.config.search.sources || []).filter((source) => {
            if (source.type === 'chatluna_model') {
                return Boolean((0, model_1.normalizeModelName)(source.model));
            }
    ...
    createSourceTask(claim, source, index) {
        const task = source.type === 'chatluna_model'
            ? this.subSearchAgent.deepSearchWithModel(claim, (0, model_1.normalizeModelName)(source.model), `fact-check-${index + 1}`, source.label || `ChatlunaSearch-${index + 1}`, (0, prompts_1.buildFactCheckToolSearchPrompt)(claim), prompts_1.FACT_CHECK_TOOL_SEARCH_SYSTEM_PROMPT)
            : this.tavilySearch.search(claim, source.tavilyApiKey, source.label || `TavilySearch-${index + 1}`);
        return (0, async_1.withTimeout)(task, Math.max(5, this.config.search.perSourceTimeout || 45) * 1000, source.type === 'chatluna_model' ? `chatluna:${source.label}` : `tavily:${source.label}`)
