...
${sourceText}`;
    ...
    formatChatlunaSearchContext(result) {
        const findings = (0, text_1.truncate)(result.findings, Math.min(this.config.factCheck.searchContextMaxChars, 500), '无可用搜索结果');
...
${sourceText}`;
    ...
    async buildChatlunaSearchContext(claim) {
        ...
        try {
            ...
            const searchResult = await (0, async_1.withTimeout)(chatlunaSearchAgent.search(claim), this.config.factCheck.searchContextTimeout, 'ChatlunaSearchContext');
...
${sourceText}`;
    ...
    async buildOllamaSearchContext(claim) {
        ...
        try {
            const searchResult = await (0, async_1.withTimeout)(this.ollamaSearchService.search(claim, 'Ollama Search 上下文', 'agent'), this.config.factCheck.searchContextTimeout, 'OllamaSearchContext');
            if (!searchResult || searchResult.failed) {
                return null;
            }
            const domains = this.extractSourceDomains(searchResult.sources);
...
function registerFactCheckTool(ctx, config) {
    ...
    const quickToolDescription = sanitizeToolDescription(config.factCheck.quickToolDescription, '用于网络搜索与事实核查。输入待核查文本，返回来源与摘要。');
    ctx.effect(() => {
        const disposables = [];
