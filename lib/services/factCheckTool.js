...
class FactCheckTool extends tools_1.StructuredTool {
    ...
    getQuickProvider() {
        const gemini = (0, model_1.normalizeModelName)(this.config.models.geminiModel);
        if (!gemini)
            return null;
        return {
            key: 'gemini',
    ...
    appendDirectSourcesBlock(output, sources) {
        ...
        const directSources = [...new Set((sources || [])
                ...
                .filter(Boolean))].slice(0, this.config.tools.maxSources);
    ...
    getToolProviders() {
        const providers = [];
        const grokModel = (0, model_1.normalizeModelName)(this.config.models.grokModel);
        if (grokModel)
            providers.push({ key: 'grok', label: 'GrokSearch', model: grokModel });
        const geminiModel = (0, model_1.normalizeModelName)(this.config.models.geminiModel);
        if (geminiModel)
