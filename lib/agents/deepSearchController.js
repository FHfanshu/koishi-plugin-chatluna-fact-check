...
class DeepSearchController {
    ...
    async search(claim) {
        ...
        for (let i = 0; i < maxIterations; i += 1) {
            ...
            try {
                const roundData = await (0, async_1.withTimeout)(this.runRound(normalizedClaim, history, roundNumber), this.config.search.perIterationTimeout * 1000, `DeepSearch 第 ${roundNumber} 轮`);
                history.rounds.push({
                    round: roundNumber,
                    plan: roundData.plan,
    ...
    async executePlan(plan) {
        ...
        const tasks = primaryQueries.map((query, index) => (0, async_1.withTimeout)(this.searchAgent.search(query), perQueryTimeoutMs, `DeepSearch查询${index + 1}:${query.focus || query.query}`));
    ...
    launchGrokSupplementalQuery(query) {
        const grokModel = (0, model_1.normalizeModelName)(this.config.models.deepSearchGrokModel) || (0, model_1.normalizeModelName)(this.config.models.grokModel);
    ...
    getPerQueryTimeoutMs() {
        const perIterationTimeout = (this.config.search.perIterationTimeout || 30) * 1000;
        const configuredPerSource = (this.config.search.perSourceTimeout || 45) * 1000;
        const budget = Math.max(5000, perIterationTimeout - 3000);
        return Math.max(5000, Math.min(configuredPerSource, budget));
    }
    ...
    collectAllSources(history) {
        return this.normalizeSourceUrls(history.rounds
            .flatMap((round) => round.results)
            .flatMap((result) => result.sources || []), []);
    }
    normalizeSourceUrls(items, fallback) {
        const urls = [...new Set((items || [])
                .flatMap((item) => (0, url_1.extractUrls)(String(item || '').trim()))
                .filter(Boolean))];
