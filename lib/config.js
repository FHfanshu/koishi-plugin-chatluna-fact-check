...
const agentSearchSchema = koishi_1.Schema.object({
    ...
    chatgptModel: koishi_1.Schema.dynamic('model').default('').description('ChatGPT 来源模型（FactCheck 与 DeepSearch 共用，留空则跳过 ChatGPT 来源）'),
    ollamaSearchMaxResults: koishi_1.Schema.number().min(1).max(10).default(5).description('Ollama Search 返回结果数（FactCheck 与 DeepSearch 共用）'),
    ollamaSearchTimeout: koishi_1.Schema.number().min(3000).max(120000).default(15000).description('Ollama Search 超时（毫秒，FactCheck 与 DeepSearch 共用）'),
    ...
    // — 搜索源上下文注入 —
    appendChatlunaSearchContext: koishi_1.Schema.boolean().default(false).description('为 fact_check 输出追加 Chatluna Search 上下文（仅附加参考，不改变最终判定流程）'),
    appendOllamaSearchContext: koishi_1.Schema.boolean().default(false).description('为 fact_check 输出追加 Ollama Search 上下文（仅附加参考，不改变最终判定流程）'),
    searchContextTimeout: koishi_1.Schema.number().min(3000).max(120000).default(12000).description('搜索上下文查询超时（毫秒）；超时仅跳过附加上下文，Chatluna/Ollama 共用'),
    searchContextMaxChars: koishi_1.Schema.number().min(200).max(8000).default(1200).description('附加的搜索上下文最大字符数，Chatluna/Ollama 共用'),
    searchContextMaxSources: koishi_1.Schema.number().min(1).max(20).default(5).description('附加的搜索来源数量上限，Chatluna/Ollama 共用'),
}).description('搜索配置');
...
const deepSearchChatlunaIntegrationSchema = koishi_1.Schema.object({
    useChatlunaSearchTool: koishi_1.Schema.boolean().default(true).description('优先尝试调用 web_search 工具执行迭代搜索'),
    usePuppeteerBrowser: koishi_1.Schema.boolean().default(false).description('允许主控计划调用 browser 工具抓取页面'),
}).description('Chatluna 搜索集成');
const apiUnifiedSchema = koishi_1.Schema.object({
    ollamaApiKey: koishi_1.Schema.string().role('secret').default('').description('Ollama API Key（从 https://ollama.com 获取，填入后即可启用 Ollama 搜索）'),
    ollamaBaseUrl: koishi_1.Schema.string().default('').description('Ollama Base URL（留空使用默认 https://ollama.com/api/web_search）'),
    ollamaEnabled: koishi_1.Schema.boolean().default(true).description('启用 Ollama 搜索（关闭后不会作为 fact_check 搜索来源）'),
...
exports.Config = koishi_1.Schema.intersect([
    koishi_1.Schema.object({
        ...
        deepSearch: koishi_1.Schema.intersect([deepSearchCoreSchema, deepSearchChatlunaIntegrationSchema]).description('DeepSearch'),
