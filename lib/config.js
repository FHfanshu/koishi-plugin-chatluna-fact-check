...
const customSourceSchema = koishi_1.Schema.object({
    provider: koishi_1.Schema.string().default('tavily').description('来源标识（例如 chatluna、tavily）'),
    model: koishi_1.Schema.dynamic('model').default('').description('Chatluna 模型（provider=chatluna 时使用）'),
...
const searchSchema = koishi_1.Schema.object({
    search: koishi_1.Schema.object({
        chatluna: koishi_1.Schema.object({
            models: koishi_1.Schema.array(koishi_1.Schema.dynamic('model').description('搜索模型')).default([]).description('Chatluna 模型列表'),
        }).description('Chatluna 模型来源'),
